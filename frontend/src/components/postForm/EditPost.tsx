...
export default function EditPost({ postId, isOpenModal, handleClose }: any) {
  ...
  const { title, subTitle, status, _id } = formData;

  useEffect(() => {
    const findPost = posts?.find((post: any) => post._id === postId);
    setFormData({
      title: !findPost?.title ? "" : findPost.title,
      subTitle: !findPost?.subTitle ? "" : findPost.subTitle,
      status: !findPost?.status ? "" : findPost.status,
      _id: !findPost?._id ? "" : findPost._id,
    });
    setShow(true);
  }, []);
